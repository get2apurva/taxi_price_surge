...
import com.nyc.surgepricing.exception.SurgepricingCustomException;
import com.nyc.surgepricing.utils.JavaSparksessionSingleton;
import com.nyc.surgepricing.utils.SurgepricingUtils;

/**
 * The Class SurgepricingCalculator. calculating surge pricing and traffic
 * congestion with historic dataset calculating supply vs demand ratio for surge
 * pricing calculation and its effect with weather
 */
public class SurgepricingCalculator implements SurgepricingConstants {

	/** The Constant LOG. */
	private static final Logger LOG = Logger.getLogger(SurgepricingCalculator.class);
	...
	public static void main(String[] args) {
		try {
			...
			Dataset<Row> yelloCabDF = deriveTripAttributes(sparkSession, getYellCabDF(sparkSession));
			...
			throw new SurgepricingCustomException(exception.getMessage());
		} finally {
			sparkSession.stop();
			sparkSession.close();
		}
	...
	public static Dataset<Row> deriveTripAttributes(SparkSession sparkSession, Dataset<Row> yelloCabDF) {
		...
		yelloCabDF = removeOutliers( yelloCabDF);
		return yelloCabDF;
	}
	 ...
	 */
	public static Dataset<Row> removeOutliers( Dataset<Row> yelloCabDF) {
		return yelloCabDF
				.filter(yelloCabDF.col(PICKUP_GEOHASH).isNotNull().and(yelloCabDF.col(DROPOFF_GEOHASH).isNotNull())
						.and(yelloCabDF.col(TRIP_DISTANCE).leq(functions.lit(200))));
	...
	public static Dataset<Row> calculateTrafficCongestion(SparkSession sparkSession, Dataset<Row> yelloCabDF) {
		...
		yelloCabDF = yelloCabDF.withColumn(TRAFFIC_LEVEL,
				functions.when(yelloCabDF.col(SPEED_DIFF).leq(0), functions.lit(1))
						...
						.when(yelloCabDF.col(SPEED_DIFF).gt(5), functions.lit(4)).otherwise(functions.lit(2)));
		return yelloCabDF.withColumnRenamed(PICKUP_GEOHASH, GEOHASH)
